...
async function scenario(serverPort) {
  ...
  await p1.waitForState(g => g.activateMainConfirmWindow == null, { label: 'confirm closed (p1)' });
  await p2.waitForState(g => g.activateMainConfirmWindow == null, { label: 'confirm closed (p2)' });
  for (const c of [p1, p2]) {
    const ld = c.lastState.players[p1.playerId].leader;
